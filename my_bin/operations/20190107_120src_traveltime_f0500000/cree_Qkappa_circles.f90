...
! r(x,z))/T), where T = 2* (outer_cut - inner_cut)
 ...
 ! On remplit ce tableau avec x,z
  ...
  write(filename,'(a,i6.6,a)') './model_create/proc',iproc-1,'_Qkappa.bin'
    ...
    do j=1,NGLL
      do i=1,NGLL
       ...
       if (mf_range_big) then
           if (mf_range) then
               vp_store(i,j,ispec) = 37.7
           else if (mf_range2) then 
               vp_store(i,j,ispec) = 42.0
           else 
               vp_store(i,j,ispec) = 40.0
           end if
       else 
           vp_store(i,j,ispec) = 37.7
       end if
...
end program cree_Qkappa_circles
