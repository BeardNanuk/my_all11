...
#        if not exists(PATH.SUBMIT + '/' + 'scratch'):
 #           unix.ln(PATH.SCRATCH, PATH.SUBMIT + '/' + 'scratch')
        ...
        call('sbatch '
                ...
                + '--nodes %d ' % PAR.nodes # get rid of nnodes by JH on Fri Mar 15 13:14:46 EDT 2019
                #+ '--ntasks-per-node=20 '
                #+ '--ntasks-per-node=%d '%PAR.NTASK ## JH Fri Jan 11 19:45:05 EST 2019
                + '--ntasks-per-node=%d '%PAR.NGPU
                + '--ntasks-per-socket=%d '%PAR.NGPU
                + '--exclusive ' # % own the node # added by JH on Mar 15 11:05:52 EDT 2019 
                + '--gres=gpu:%d '%PAR.NGPU 
                + '--time=%d '%PAR.WALLTIME
