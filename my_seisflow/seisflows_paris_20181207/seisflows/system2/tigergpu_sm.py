...
class tigergpu_sm(custom_import('system', 'slurm_sm')):
  ...
  #  def run(self, classname, method, hosts='all', **kwargs):
    ...
    #            + join(pkgpath('seisflows'), 'system/wrappers/run') + ' '
        call('/scratch/gpfs/etienneb/seisflows_old_casse_le_19_11_2017/seisflows/system/wrappers/dsh2' + ' '
                + str(PAR.NTASK) + ' '
             '/scratch/gpfs/etienneb/seisflows/seisflows/system/wrappers/run' + ' '
                + PATH.OUTPUT + ' '
                + classname + ' '
                + method + ' '
                + PAR.ENVIRONS)
