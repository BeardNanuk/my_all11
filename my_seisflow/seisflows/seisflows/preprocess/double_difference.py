...
class double_difference(custom_import('preprocess', 'base')):
    ...
    def write_residuals(self, path, syn, dat):
        ...
        #for i in range(nr):  #JH comment out on 20190430
        #    for j in range(i):  ##JH comment out on 20190430
        #        dist[i,j] = self.distance(rx[i], rx[j], ry[i], ry[j]) #JH comment out on 20190430

        # calculate traveltime lags between stations pairs
        for i in range(nr):
            for j in range(i):
                #if dist[i,j] > PAR.DISTMAX: #JH comment out on 20190430 
                #    continue    #JH comment out on 20190430
    ...
    def write_adjoint_traces(self, path, syn, dat, channel):
        ...
        # '-' was added by JH on Thu May 16 14:59:43 EDT 2019
        #adj2 = -adj 
