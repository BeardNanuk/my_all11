...
def Waveform_norm(syn, obs, nt, dt):
    from numpy.linalg import norm
    syn_norm=norm(syn,axis=0)
    wadj = (syn_norm - obs)
    return wadj

def Waveform_norm2(syn, obs, nt, dt):
    from numpy.linalg import norm
    syn_norm=norm(syn)
    obs_norm=norm(obs)
    wadj = (syn/syn_norm - obs/obs_norm)
    return wadj
...
def Waveform_att(syn, obs, nt, dt):
    ...
    freq = _np.ones(nt)
    freq[1:nt/2] = _np.linspace(1,fmax, num=(nt-1)/2)
    freq[nt/2:] = _np.linspace(-fmax,-1, num=nt/2)
    #reference frequency
    freq_ref = 100000.0
    wadj = ifft(((2.0/_np.pi)*_np.log(abs(freq)/freq_ref)- 1j*_np.sign(freq))*tf_adj)
