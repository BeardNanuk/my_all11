...
class slurm_lg_gpu_ze(custom_import('system', 'base_lg')):
    ...
    def submit(self, workflow):
        ...
        call('sbatch '
                + '%s ' % PAR.SLURMARGS
                #+ '--job-name=%a '# % PAR.TITLE # add by JH on Sat Mar 23 14:17:35 EDT 2019 
    ...
    def run(self, classname, method, *args, **kwargs):
        ...
        stdout = check_output(
                   ...
                   #+ '--job-name=%s ' % PAR.TITLE
                   + '--job-name=%s ' % '%a' # % PAR.TITLE # add by JH on Sat Mar 23 14:17:35 EDT 2019 
                   + '--nodes=%d ' % PAR.NNODES_PERTASK # added by JH on Tue Mar 26 14:48:53 EDT 2019
                   #+ '--ntasks-per-node=%d ' % PAR.NODESIZE
	           + '--gres=gpu:%d '%PAR.NGPU
                   + '--mem-per-cpu=%d ' %30000
                   + '--time=%d ' % PAR.TASKTIME
        ...
        # isdone, jobs = self.job_array_status(classname, method, jobs) ## added by JH
    ...
    def job_array_status(self, classname, method, jobs):
        ...
        for job in jobs:
            state = 'notready'           
            #print('"notready" state is initiated for job ', job) 

            while state=='notready': 
                state = self.job_status(job)
                #print('job, state: %s and %s' % (job,state))             
                if state=='notready':
                    time.sleep(30)
                    print('I sleeped for 30 sec')
    ...
    def job_status(self, job):
        ...
        for line in lines:
            if not line: 
                print(' line is empty, for job: ', job) 
            else:
                #print('line: ',line)
                if line.split()[0]==job:
                    state = line.split()[1]
