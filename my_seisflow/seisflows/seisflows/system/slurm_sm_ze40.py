...
class slurm_sm_ze40(custom_import('system', 'base')):
    ...
    def submit(self, workflow):
        ...
        # submit workflow
        #ntasks = PAR.NTASK  # added by JH on Mon Jan  7 06:51:27 EST 2019
        #ntasks = PAR.NTASK * PAR.NPROC # commented out by JH on Sat Dec  8 22:28:27 UTC 2018
        call('sbatch '
                + '%s ' %  PAR.SLURMARGS
                + '--job-name=%s '%PAR.TITLE
                + '--output=%s '%(PATH.WORKDIR +'/'+ 'output.log')
                #+ '--cpus-per-task=%d '%PAR.NPROC
                + '--nodes=%d '  % PAR.nodes  # added by JH on Sun Jan  6 10:11:35 UTC 2019
                #+ '--nodes=%d '  % ceil(ntasks/40) # PAR.nodes  # added by JH on Sat Dec  8 22:28:27 UTC 2018
                + '--ntasks-per-node=40 ' # % PAR.NTASK
                #+ '--ntasks-per-node=%d '  % ntasks
                + '--exclusive ' # % own the node # added by JH on Tue Mar 12 14:21:01 EDT 2019
                + '--time=%d '%PAR.WALLTIME
