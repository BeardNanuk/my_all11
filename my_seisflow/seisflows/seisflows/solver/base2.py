...
class base2(object):
    ...
    def setup(self):
        ...
        if PATH.DATA:
            ...
            print('now copying observed data')
            src = glob(PATH.DATA +'/obs/'+ self.source_name +'/'+ '*')
            dst = 'traces/obs/'
            unix.cp(src, dst)
            #print('now copying synthetics')
            #src = glob(PATH.DATA +'/syn/'+ self.source_name +'/'+ '*')
            #dst = 'traces/syn/'
            #unix.cp(src, dst)
            #print('now copying lastframe*.bin')
            #src = glob(PATH.DATA +'/OUTPUT_FILES/'+ self.source_name +'/'+ 'last*')
            #dst = 'OUTPUT_FILES'
            #unix.cp(src, dst)
            ...
            ###self.generate_mesh(  ### commented out by JH on Sat Mar  9 21:39:56 EST 2019
            self.generate_mesh(
    ...
    def save(self, dict, path, parameters=['vp','vs','rho','Qkappa'], prefix='', suffix=''):
